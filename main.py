...
def run_single_iteration(snr_db, P, K, Q, E_TX, MAX_ITER):
    ...
    # --- Curve 2: WMMSE1 (10 Random Init + Convergence) ---
    best_rate = 0
    NUM_RAND_INIT = 10
    MAX_CONV_ITER = 50
    EPSILON = 1e-3
    ...
    return r_txmf, r_wmmse1, r_wmmse2
...
if __name__ == "__main__": # 윈도우/리눅스 멀티프로세싱 필수 구문
    ...
    results = {
        ...
        "WSRBF-WMMSE2 (10 itermations - TxMF)": []
    }

    current_params = {"P": p.P, "K": p.K, "Q": p.Q, "SNR_RANGE": list(p.SNR_DB_RANGE)}
    ...
    for snr_db in tqdm(p.SNR_DB_RANGE, desc="SNR Loop"):
        ...
        r_txmf_list, r_wmmse1_list, r_wmmse2_list = zip(*parallel_results)
        
        # 평균 계산 및 저장
        results["TxMF"].append(np.mean(r_txmf_list))
        results["WSRBF-WMMSE1 (convergence/10 random init)"].append(np.mean(r_wmmse1_list))
        results["WSRBF-WMMSE2 (10 itermations - TxMF)"].append(np.mean(r_wmmse2_list))
